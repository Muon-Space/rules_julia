...
_JLL_PKG_BUILD_FILE_CONFIG_SETTING = """\
config_setting(
    name = "{name}",
...
_JLL_PKG_BUILD_FILE_ARTIFACT_SELECT = """\
...
def _create_artifact_repos(module_ctx, hub_name, package_name, package_uuid, artifacts):
    ...
    for artifact_name, artifact_info in artifacts.items():
        ...
        build_file_content = _ARTIFACT_BUILD_FILE.format(
...
def _generate_jll_build_file(package_name, deps, hub_name, repo_name, version, uuid, artifact_repos):
    ...
    """
    build_content = _JLL_PKG_BUILD_FILE_HEADER.format(
        dependencies = deps,
    )
    ...
    for platform_key in sorted(all_platforms):
        if platform_key in _PLATFORM_TO_CONSTRAINTS:
            ...
            build_content += _JLL_PKG_BUILD_FILE_CONFIG_SETTING.format(
                name = sanitized_name,
    ...
    for artifact_name, platform_repos in artifact_repos.items():
        ...
        build_content += _JLL_PKG_BUILD_FILE_ARTIFACT_SELECT.format(
            artifact_name = artifact_name,
            select_cases = "\n".join(select_cases),
        )
    ...
    build_content += _JLL_PKG_BUILD_FILE_FOOTER.format(
        name = package_name,
        hub_name = hub_name,
        repo_name = repo_name,
        version = version,
